  ...
  return (
    <MainLayout>
      <div className="page-container py-8">
        <PageHeader
          title="Panchayat Members & Staff"
          description="Meet your elected representatives and panchayat staff"
          icon={Users}
        />
        ...
        {sarpanch && (
          <section className="mb-8">
            <h2 className="text-lg font-semibold text-muted-foreground mb-4">Sarpanch</h2>
            <FeaturedMemberCard member={sarpanch} />
          </section>
        )}
        ...
        {otherLeaders.length > 0 && (
          <section className="mb-8">
            <h2 className="text-lg font-semibold text-muted-foreground mb-4">Panchayat Leadership</h2>
        ...
        {wardMembers.length > 0 && (
          <section className="mb-8">
            <h2 className="text-lg font-semibold text-muted-foreground mb-4">Ward Members</h2>
        ...
        {staff.length > 0 && (
          <section>
            <h2 className="text-lg font-semibold text-muted-foreground mb-4">Panchayat Staff</h2>
            <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {staff.map((member) => (
                <MemberCard key={member.id} member={member} compact />
              ))}
    ...
    <article className="card-elevated overflow-hidden">
      <div className="md:flex">
        ...
        <div className="flex-1 p-6">
          <div className="mb-4">
            <span className="text-xs bg-primary text-primary-foreground px-2 py-1 rounded uppercase font-medium">
          ...
          <div className="space-y-2 text-sm text-muted-foreground mt-4">
            {member.phone_public && (
              <div className="flex items-center gap-2">
                ...
                <a href={`tel:${member.phone_public}`} className="hover:text-foreground transition-colors">
                  {member.phone_public}
                </a>
              </div>
            )}
            {member.email_public && (
              <div className="flex items-center gap-2">
                ...
                <a href={`mailto:${member.email_public}`} className="hover:text-foreground transition-colors">
                  {member.email_public}
                </a>
              </div>
            )}
            <div className="flex items-center gap-2">
              ...
              <span>
                Term: {new Date(member.term_start).getFullYear()} - {new Date(member.term_end).getFullYear()}
              </span>
            </div>
          </div>
        </div>
...
function MemberCard({ member, compact }: { member: Member; compact?: boolean }) {
  const roleColors: Record<string, string> = {
    DEPUTY_SARPANCH: 'bg-success/10 text-success',
    ...
    <article className={cn('card-elevated p-4', compact ? '' : 'p-5')}>
      <div className="flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          ...
          {member.ward_no && (
            <p className="text-xs text-muted-foreground mt-1">Ward {member.ward_no}</p>
          )}
          {!compact && member.phone_public && (
            <div className="flex items-center gap-1.5 mt-2 text-sm text-muted-foreground">
              <Phone className="w-3.5 h-3.5" />
              <a href={`tel:${member.phone_public}`} className="hover:text-foreground">
                {member.phone_public}
              </a>
            </div>
          )}
